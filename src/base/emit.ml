...
let f oc (Prog(data, fundefs, e)) =
  ...
  Printf.fprintf oc "\tpush\t%%rax\n";
  Printf.fprintf oc "\tpush\t%%rbx\n";
  Printf.fprintf oc "\tpush\t%%rcx\n";
  Printf.fprintf oc "\tpush\t%%rdx\n";
  Printf.fprintf oc "\tpush\t%%rsi\n";
  Printf.fprintf oc "\tpush\t%%rdi\n";
  Printf.fprintf oc "\tpush\t%%rbp\n";
  Printf.fprintf oc "\tmov\t32(%%esp),%s\n" reg_sp;
  Printf.fprintf oc "\tmov\t36(%%esp),%s\n" regs.(0);
  Printf.fprintf oc "\tmov\t%s,%s(%%rip)\n" regs.(0) reg_hp;
  stackset := S.empty;
  stackmap := [];
  g oc (NonTail(regs.(0)), e);
  Printf.fprintf oc "\tpop\t%%rbp\n";
  Printf.fprintf oc "\tpop\t%%rdi\n";
  Printf.fprintf oc "\tpop\t%%rsi\n";
  Printf.fprintf oc "\tpop\t%%rdx\n";
  Printf.fprintf oc "\tpop\t%%rcx\n";
  Printf.fprintf oc "\tpop\t%%rbx\n";
  Printf.fprintf oc "\tpop\t%%rax\n";
  Printf.fprintf oc "\tret\n";
